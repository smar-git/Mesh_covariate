...
author:
  ...
  - name: Erin Bryce
    affil: 2
    main: true
    email: '2313549b@student.gla.ac.uk'
...
In ecology, spatial point processes (SPP) are a statistical methodology that are becoming increasingly popular through their implementation using an inference technique, the integrated nested Laplace approximation (INLA) alongside the stochastic partial differential equation approach (SPDE). Key to this approach is the representation of a spatially continuous Gaussian random field, defined over a triangular mesh spanning the spatial domain, as the solution to an SPDE. Crucial to model implementation is the definition of both the mesh in conjunction with the spatial information/covariates available, and the associated integration points.     

<!-- Erin update: just an idea - not sure it made it shorter or less technical though! -->
...
The Gaussian random field and spatial covariates are evaluated at the same locations, the integration points, across the spatial domain. As SPP are models continuous in space, the continuity of the spatial covariate and spatial random effect are approximated by interpolation between integration points. Therefore, the model will only _see_ the spatial covariates at these defined integration points. If a covariate _actually_ has a range much smaller than the distance between integration points, the model will fail to provide reliable inference.  

<!-- Erin update: just tried to make it sound less like an INLA/SPDE problem -->
...
<!-- Is there a way to centre this figure and the covariates one? Ive used fig.align in the chunk options but they are weirdly aligned to the right of the column  -->
...
```{r, covars, fig.cap='Three covariates randomly generated. Covariate 1 has a range of 10, Covariate 2 has a range of 50, and covariate 3 has a range of 600', fig.align='left', out.width="130%"}
