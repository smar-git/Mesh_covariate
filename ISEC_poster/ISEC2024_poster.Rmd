...
main_bottomsize: 0.1
title: '**Spatial Point Processes modelling with INLA: are finer covariate resolutions always better?**'
author:
...
main_findings:
  - "**Innacurate covariates resolution within INLA/SPDE mesh definition lead to unreliable numerical integration and biased parameter estimates**."
...
This is the `iheiddown_betterport` template for the `{iheiddown}`
[@iheiddown] package! Based on the great `{posterdown}` package, it
provides the user with an IHEID version of the template by default. The
original `{posterdown}` template was inspired by the twitter thread of
[Mike
Morrison](https://mobile.twitter.com/mikemorrison/status/1110191245035479041).Please
check also this [wiki
page](https://github.com/brentthorne/posterdown/wiki/posterdown_betterport)
with another similar package: Posterdown [@posterdown].
